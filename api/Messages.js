export const getAllMessagesForAdmin = async () => {
    try {
        const response = await fetch('https://acampamento-jovem-backend.vercel.app/mural_mensagem/', {
            credentials: "include"
        })
...
export const getAllMessagesForUser = async () => {
    try {
        const response = await fetch('https://acampamento-jovem-backend.vercel.app/mural_mensagem/find')
        const data = await response.json()
        if (!response.ok) {
            throw new Error(data.message || "Erro ao buscar todas as mensagens")
        }
...
export const updateMessages = async (id, isActive) => {
    try {
        const response = await fetch(`https://acampamento-jovem-backend.vercel.app/mural_mensagem/${id}`, {
            credentials: "include",
            method: "PATCH",
            headers: {
